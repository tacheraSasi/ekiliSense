...
$get_info = mysqli_query(
    $conn,
    "SELECT * FROM schools WHERE unique_id = '$school_uid'"
);
...
{
    $teacher = mysqli_real_escape_string(
        $conn,
        $_POST["choosen-subject-teacher"]
    ); #TODO:remove the id from the frontend
    $class = mysqli_real_escape_string($conn, $_POST["class"]);
    ...
    $insert_subject = mysqli_query(
        $conn,
        "INSERT INTO subjects (subject_uid,subject_name,School_unique_id,class_id,teacher_id)
...
function addTeacher($conn, $school_uid, $school_name)
{
    ...
    if (mysqli_num_rows($check_query) > 0) {
        ...
        $insert_teacher_query = mysqli_query(
            $conn,
...
function addClass($conn, $school_uid)
{
    ...
    if (!$check) {
        ...
        $query = mysqli_query(
            $conn,
            "INSERT INTO classes (school_unique_id, Class_name)
...
function addClassTeacher($conn, $school_uid)
{
    ...
    $get_class_id_query = mysqli_query(
        $conn,
    ...
    $get_teacher_id_query = mysqli_query(
        $conn,
    ...
    if ($check) {
        ...
        $insert_query = mysqli_query(
            $conn,
            "INSERT INTO class_teacher (school_unique_id, Class_id, teacher_id)
...
function sendMail($email, $name, $otp, $school_name)
{
    ...
    <body style="max-width: 600px; margin: 0 auto; border-radius: 1rem; padding: 1rem; color: #fff; background-color: rgba(35, 51, 29, 0.77);">
        ...
        ',</p>
            <p style="color: #eee;">
                Congratulations! You have been successfully registered as a teacher at <strong>' .
        $school_name .
        '</strong>.
            </p>
            <p style="color: #eee;">
            ...
            <div style="text-align: center; margin-top: 20px;">
                <a href="https://auth.ekilie.com/sense/verify/?otp=' .
        $otp .
        "&user=" .
        $email .
        '" style="background-color: #8cc75c; color: #fff; padding: 10px 20px; border-radius: 5px; text-decoration: none;">Verify Account</a>
            </div>
            <p style="color: #eee; margin-top: 20px;">
                Once your account is verified, you will unlock full access to our platform and resources, empowering you to elevate your teaching experience at <b> ' .
        $school_name .
